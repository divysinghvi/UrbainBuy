...
class ShoppingCart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double subtotal = cart.cartItems.fold(
        0, (sum, item) => sum + double.parse(item['price']));
    double discount = cart.cartItems.fold(
        0, (sum, item) => sum + (double.parse(item['price']) * 0.1));
    double vat = 1.3;
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              icon: Icon(Icons.notifications_active_outlined, color: Colors.black), // Set icon color to black
              onPressed: () {
